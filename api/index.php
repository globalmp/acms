...
$content_json = file_get_contents( "http://127.0.0.1/api/grades.php" );

preg_match_all("/\"id\":([0-9]{1,5}),/",$content_json,$content_json);



for($i=0;$i<$content_json[1];$i++)
{
	sleep(rand(1,3));
$content = file_get_contents( "http://metal-calculator.ru/api/grades/".(int)$content_json[1][$i]."?access_token=Fs8fj5bF21&license_key=undefined&partner_key=4HKJKR9922" );


$fd = fopen("grades/".$content_json[1][$i],"w+");
fwrite($fd,$content);
fclose($fd);
...
function getHTML( $url, $proxy = false, $post = false, $referer = false ){
		$robot = curl_init();
		$request_headers[] = 'User-Agent: Mozilla/5.0 (Windows NT 6.1; rv:43.0) Gecko/20100101 Firefox/43.0';
	    $request_headers[] = 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8';
	    $request_headers[] = 'Accept-Language: ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3';
		curl_setopt($robot, CURLOPT_HTTPHEADER, $request_headers);
		if( $referer != false ) curl_setopt($robot, CURLOPT_REFERER, $url);
		curl_setopt($robot, CURLOPT_TIMEOUT, 15);
		curl_setopt($robot, CURLOPT_URL, $url);
		curl_setopt($robot, CURLOPT_RETURNTRANSFER, true); 
		curl_setopt($robot, CURLOPT_FOLLOWLOCATION, 1);
		curl_setopt($robot, CURLOPT_COOKIEFILE, dirname ( __FILE__ ) . "/f-other.txt");
		curl_setopt($robot, CURLOPT_COOKIEJAR, dirname ( __FILE__ ) . "/f-other.txt");
		curl_setopt($robot, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($robot, CURLOPT_HEADER, 0);
		$content = curl_exec($robot);
		$info = curl_getinfo($robot);
		curl_close($robot);
		//if(!@preg_match('/utf-8/i',$info['content_type'])) $content = iconv( "cp1251", "utf-8", $content );
		return $content;
	}
